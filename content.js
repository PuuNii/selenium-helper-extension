...
//  CONSTANTS
// ============================================================
const OVERLAY_ID    = '__selenium_helper_overlay__';
const STORAGE_KEY   = 'seleniumHelperPendingElement';
...
function onClick(e) {
  ...
  document.removeEventListener('mousedown', onSuppressDown, true);

  // Collect and broadcast element data
  const data = collectElementData(e.target);
...
function collectElementData(rawEl) {
  ...
  const textContent = sanitizeText(el.textContent);
  ...
  return {
    ...
    placeholder, title, role, type, className,
...
function startSelectionMode() {
  if (selectionMode) return; // Guard against double-activation
  selectionMode = true;
  createOverlay();
  document.addEventListener('mousemove', onMouseMove, true);
  document.addEventListener('click',     onClick,     true);
  document.addEventListener('mousedown', onSuppressDown, true);
}
